...
async function main() {
  ...
  const teacher1 = await prisma.user.upsert({
    where: { email: 'teacher1@crm.uz' },
    update: {},
    create: {
      name: 'Sherzod Asadov',
      email: 'teacher1@crm.uz',
  ...
  // Create SMM user
  const smmPassword = await bcrypt.hash('smm123', 10);
  const smm1 = await prisma.user.upsert({
    where: { email: 'smm1@crm.uz' },
    update: {},
    create: {
      name: 'Media SMM',
      email: 'smm1@crm.uz',
